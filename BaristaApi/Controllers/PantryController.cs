...
namespace BaristaApi.Controllers
{
    ...
    public class PantryController: ControllerBase
    {
        ...
        [Route("{id}/beverages")]
        [HttpGet]
        public async Task<IEnumerable<BeverageDTO>> GetBeveragesAsync(string id)
        {
        ...
        [Route("{id}/beverage/{beverageId}/orders")]
        [HttpGet]
        public async Task<IActionResult> GetBeverageOrdersAsync(string id, string beverageId)
        {
            var pantry = await _pantryRepository.GetAsync(id);
            if (pantry == null) return NotFound();
            Guid.TryParse(beverageId, out Guid gBevId);
            var orders = pantry.Beverages.FirstOrDefault(b => b.BeverageId.Equals(gBevId)).Orders.OrderBy(o => o.OrderDate);
